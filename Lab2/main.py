...
for i in range(len(k1m_vals)):
    x_val = x0_func(y_k, k3val, k3m_val)
    k1_val = k1_sol0_func(y_k, k1m_vals[i], k2val, k3val, k3m_val)

    x_val, y_p, k1_val = zip(*[(x, y, z) for x, y, z in zip(x_val, y_k, k1_val) if 0 <= x <= 1])

    plt.subplot(5, 1, i + 1)
    plt.plot(k1_val, x_val, color='red', label='x(k1)')
    plt.plot(k1_val, y_p, color='green', label='y(k1)')
...
for i in range(len(k3m_vals)):
    x_val = x0_func(y_k, k3val, k3m_vals[i])
    k1_val = k1_sol0_func(y_k, k1m_val, k2val, k3val, k3m_vals[i])
...
for j in range(len(k3m_vals)):
    x_val = x0_func(y_k, k3val, k3m_vals[j])
    k1_val = k1_sol0_func(y_k, k1m_val, k2val, k3val, k3m_vals[j])

    x_val, y_p, k1_val = zip(*[(x, y, z) for x, y, z in zip(x_val, y_k, k1_val) if 0 <= x <= 1])
    k1_bif, y_bif, x_bif = get_bief(x_val, y_p, k1_val, k1m_val, k2val, k3val, k3m_vals[j])
